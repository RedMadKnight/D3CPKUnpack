#!/usr/bin/env python3
"""
D3CPK Extractor — reference parser for Blizzard's CPK archive format
as used in the console releases of Diablo III.

Supports both archive variants present in the shipped data:
  - v6 (big-endian)
  - v7 (little-endian)

Endianness is auto-detected from the magic word (0xA1B2C3D4).

The extractor handles three details not present in naive zlib-only
CPK readers, all established by round-tripping against the shipped
archives:

  (1) Chunks whose `zsize_raw` has bit 0x8000 set are not compressed
      with an alternative algorithm — they are encrypted zlib. The
      encryption is a stateful stream cipher with a fixed initial
      state and ciphertext feedback; see `decrypt_stream_cipher_chunk`
      for its derivation. The pipeline for encrypted chunks is:
      decrypt, then zlib.decompress.

  (2) When zlib decompresses a chunk to fewer bytes than the chunk
      header's `decomp_size` declares, the shortfall must be
      zero-padded so that global-stream offsets stay aligned with
      the Locations table. Chunks do not span sector boundaries.

  (3) Some archives declare a `decomp_fs` in the header that exceeds
      the sum of physically present chunk payloads. Extend the global
      stream with zeros up to `decomp_fs` so that orphaned
...
if len(sys.argv) < 2:
    print("Usage: python3 d3cpk_extractor.py <file.cpk> [<output_dir>]")
    sys.exit(1)
...
if magic_be == 0xA1B2C3D4:
    ENDIAN = '>'
    ENDIAN_NAME = 'big-endian (CPK v6)'
elif magic_le == 0xA1B2C3D4:
    ENDIAN = '<'
    ENDIAN_NAME = 'little-endian (CPK v7)'
else:
...
def read_bits_be(buf, bit_pos, bit_count):
    ...
    This is the SAME function used for both the big-endian (v6) and
    little-endian (v7) archive variants. Empirical testing showed that
    although word-level values (magic, sizes, offsets) swap byte order
    between the two variants, the bit-stream packing in SFI / Locations /
    CompSectorToDecomp tables remains MSB-first within each byte in both.
    """
    result = 0
...
comp_sector_count = (comp_sector + len(data) - 1 - read_sector * header_sector) // comp_sector
ds2cs_bc = highest_bit(comp_sector_count)
...
if file_count > 10_000_000 or decomp_fs > 100_000_000_000:
    ...
    print(f"    If this is the LE/v7 build, the bit-packing byte order")
...
cs2do_bytes = (comp_sector_count * LOC_BC + 7) // 8
cs2do_block = data[pos : pos + cs2do_bytes]
pos += cs2do_bytes
cs2do = []
bp = 0
for i in range(comp_sector_count):
    cs2do.append(read_bits(cs2do_block, bp, LOC_BC)); bp += LOC_BC
...
# ====================================================================
# CPK chunk cipher (stream cipher with ciphertext feedback)
# ====================================================================

STREAM_CIPHER_INITIAL_STATE = 0x872DCDA7A97A7EE1

def decrypt_stream_cipher_chunk(ciphertext):
    """
    Decrypt an encrypted CPK chunk payload.

    Derivation of the cipher (known-plaintext attack against zlib
    prefixes):

    Every plaintext behind an encrypted chunk is a zlib stream, which
    begins with a highly constrained prefix — magic byte 0x78, a flevel
    byte drawn from {0x01, 0x5E, 0x9C, 0xDA}, and DEFLATE framing whose
    first length fields are further constrained (BTYPE=01 for fixed
    Huffman in typical compressor output, or a tightly bounded stored-
    block length). This leaks roughly 8 bytes of the per-chunk
    keystream directly from any encrypted chunk.

    Cross-chunk analysis of those leaked keystream bytes establishes:

      * Every encrypted chunk starts from the same 64-bit initial
        state. No per-chunk IV or nonce is used.
      * The state-update rule is deterministic in the current
        ciphertext byte. Regressing the leaked keystream bytes
        against candidate update rules across many chunks yields:

            state_{n+1} = ((c_n & 0xFF) << 56) | (state_n >> 8)

        — ciphertext feedback into the high byte of the state, with
        the rest shifting right. Confirmed by full round-trip
        extraction of every shipped archive.
    ...
    """
    state = STREAM_CIPHER_INITIAL_STATE
    pt = bytearray()
    for c in ciphertext:
...
def decompress_sector(sec_idx):
    ...
    while p + 6 <= sec_end:
        ...
        if encrypted:
            ...
            try:
                chunk_data = decrypt_stream_cipher_chunk(chunk_data)
...
# Some CPK archives have SFI entries for orphaned/unused files whose data was
# never written. decomp_fs in the header promises more bytes than physically
# exist in the last sectors. Extend the stream with zeros so these files can
# still be extracted (as zero-filled) instead of being listed as Failed.
